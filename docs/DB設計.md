...
| v1.3       | `users` テーブルに `display_name`（VARCHAR(50), NOT NULL）を追加（BE①提案・決定事項・プッシュ済み）。グループメンバー一覧（`GET /v1/groups/{id}/members`）等での表示名として使用する                                                                                                                                                      |
| **v1.4**   | **①issue #73対応：`users` テーブルに `email_notify_enabled`（BOOLEAN, NOT NULL, DEFAULT true）を追加。`notification_schedules.status` の取りうる値に `skipped`（メール通知OFFユーザーへの意図的な未送信）を追加。②issue #20で実装済みだった `app_notifications.user_id`（個人ごとの既読管理用カラム）の記載漏れをあわせて修正（最新版）** |
...
`plan_status` / `monthly_scan_count` / `remind_days_before` / `display_name`（v1.3で追加） / **`email_notify_enabled`（v1.4で追加）** の5カラムは `users` テーブルに含まれている（3.1節参照）。API設計書の `GET /v1/users/me`（取得）・`PATCH /v1/users/me`（更新）が対応するエンドポイントであり、別テーブルでの管理は行わない。
...
### ⑥ 【v1.4で追加】users.email_notify_enabled とリマインド設定の関係について

リマインド設定（`remind_days_before` / `email_notify_enabled`）は `users` テーブル（個人）に紐づくため、同じグループ内のメンバーであっても、各自バラバラの設定があり得る（例：ある人はメール希望、別の人はアプリ内通知のみ）。これは設計上正しい挙動であり、グループ単位で統一する必要はない。

`email_notify_enabled` のデフォルト値は `true`（メール＋アプリ内通知）。既存ユーザー行には `server_default` を用いて一括で `true` を投入するため、`display_name` のときのような3段階（NULL許容→UPDATE→NOT NULL化）のマイグレーションは不要（1回の`add_column`で完結する）。

### ⑦ 【v1.4で追加】notification_schedules.status に skipped を追加した理由について

`email_notify_enabled=false` のユーザーに対しては、リマインドメールを意図的に送信しない。この場合の `status` をどう扱うかが論点になった。

既存の4値（`pending` / `sent` / `failed` / `cancelled`）には、それぞれ以下の理由で当てはめられなかった。

- `pending`のまま：Celery Beatが毎分このレコードを拾い続けてしまう
- `failed`：「送信を試みて失敗した」という意味になり、実態（意図的な未送信）と矛盾する
- `cancelled`：「ユーザー操作（済スタンプ等）によるキャンセル」を表す既存の意味と衝突する
- `sent`：「送信成功」の意味と矛盾する

そのため、5つ目の値として `skipped`（メール通知OFF設定により意図的にスキップ）を新設した。`status` カラムはDB側でCHECK制約やEnum型による制約を持たせておらず、`VARCHAR(20)`に値を追加するだけのコード変更で対応可能（マイグレーション不要）。
...
| 物理テーブル名           | 論理テーブル名         | 概要                                                                                                                                                       |
| ------------------------ | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `users`                  | ユーザー情報           | アプリ登録者のアカウント情報・プラン状態を管理。**【v1.3】表示名（`display_name`）を追加。【v1.4】メール通知ON/OFF設定（`email_notify_enabled`）を追加。** |
...
### 3.1. users（ユーザー情報）【v1.4で変更】

Firebase Authと連携するユーザーの基本情報、プラン状態、共通リマインド設定を一括管理する。**`display_name` / `plan_status` / `monthly_scan_count` / `remind_days_before` / `email_notify_enabled` はこのテーブルで管理する。**
...
| 表示名               | `display_name`             | VARCHAR(50)  | NOT NULL         | -                   | 【v1.3追加】グループメンバー一覧（UI-006-G）等で表示する名前。BE①提案・決定事項としてプッシュ済み。                                                                     |
...
| **メール通知ON/OFF** | **`email_notify_enabled`** | **BOOLEAN**  | **NOT NULL**     | **`true`**          | **【新規追加・v1.4】falseの場合、メールリマインドを送信せずアプリ内通知のみとする（issue #73）。アプリ内お知らせ（`app_notifications`）は本設定の影響を受けず常時ON。** |
...
>
> ⚠️ **マイグレーション時の注意（v1.4）**：`email_notify_enabled`は`server_default=true`を指定して追加するため、`display_name`のような3段階の移行手順は不要（詳細は0章⑥参照）。
...
### 3.7. app_notifications（アプリ内お知らせデータ）【v1.4で記載修正】
...
| **宛先ユーザーID** | **`user_id`**  | **UUID**  | **FOREIGN KEY, NOT NULL** | **-**               | **`users.id` へ接続。通知の宛先（個人ごとの既読管理に使用）。issue #20の既知の不具合対応として実装済みだったが、本書への記載が漏れていたため v1.4 で追記。** |
...
> インデックス：`(group_id, created_at)` に加え、`user_id` 単体にもインデックスを付与する（4章参照）。
...
| ステータス       | `status`        | VARCHAR(20) | NOT NULL              | `'pending'`         | `'pending'` / `'sent'` / `'failed'` / `'cancelled'` / **`'skipped'`（v1.4追加）**                 |
...
>
> **【v1.4追加】`skipped` について**：対象ユーザーの `users.email_notify_enabled` が `false` の場合、Celery Workerはメールを送信せず、当該レコードの `status` を `'pending'` から直接 `'skipped'` へ更新する（`'sent'`・`'failed'`・`'cancelled'` のいずれとも意味が異なるため新設。詳細は0章⑦参照）。`status` カラムはDB側でCHECK制約・Enum型による制限を設けていないため、この追加にマイグレーションは不要（コード変更のみ）。
